...
(function(d){var c=d.tools.tooltip;c.dynamic={conf:{classNames:"position-top position-right position-bottom position-left",top:{offset:[0,0]},right:{offset:[0,0]},bottom:{offset:[0,0]},left:{offset:[0,0]}}};function b(h){var e=d(window);var g=e.width()+e.scrollLeft();var f=e.height()+e.scrollTop();return[h.offset().top<=e.scrollTop(),g<=h.offset().left+h.width(),f<=h.offset().top+h.height(),e.scrollLeft()>=h.offset().left]}function a(f){var e=f.length;while(e--){if(f[e]){return false}}return true}d.fn.dynamic=function(f){if(typeof f=="number"){f={speed:f}}f=d.extend({},c.dynamic.conf,f);var e=f.classNames.split(/\s/),g;this.each(function(){var h=d(this).tooltip().onBeforeShow(function(l,n){var k=this.getTip(),j=this.getConf();if(!g){g=[j.position[0],j.position[1],j.offset[0],j.offset[1],d.extend({},j)]}d.extend(j,g[4]);j.position=[g[0],g[1]];j.offset=[g[2],g[3]];k.css({visibility:"hidden",position:"absolute",top:n.top,left:n.left}).show();var i=b(k);if(!a(i)){if(i[0]){var m=d.extend({},j.offset);d.extend(true,j,f.bottom);j.position[0]="bottom";j.offset[1]=m[1];k.addClass(e[2])}if(i[2]){var m=d.extend({},j.offset);d.extend(true,j,f.top);j.position[0]="top";j.offset[1]=m[1];k.addClass(e[0])}if(i[1]){var m=d.extend({},j.offset);d.extend(true,j,f.left);j.position[1]="left";j.offset[1]+=this.getTrigger().outerWidth();j.offset[0]=m[0];k.addClass(e[3])}if(i[3]){var m=d.extend({},j.offset);d.extend(true,j,f.right);j.position[1]="right";j.offset[1]-=this.getTrigger().outerWidth();j.offset[0]=m[0];k.addClass(e[1])}}k.css({visibility:"visible"}).hide()});h.onBeforeShow(function(){var j=this.getConf(),i=this.getTip();setTimeout(function(){j.position=[g[0],g[1]];j.offset=[g[2],g[3]]},0)});h.onHide(function(){var i=this.getTip();i.removeClass(f.classNames)});ret=h});return f.api?ret:this}})(jQuery);
