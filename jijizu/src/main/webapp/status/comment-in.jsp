...
<title>收到的评论-集集组是一个平台，方便家长组织好友，带着孩子们聚在一起，玩耍、交流、分享。</title>
   ...
   <%@include file="/status/inc/left-tab-inc.jsp" %>
    ...
    <div class="plc_main">
      <div class="W_main_a">
        <div class="W_main_c">
        	<div class="tab">
            	<a href="/commentIn.jspa" class="hover">收到的评论</a>
                <a href="/commentOut.jspa">发出的评论</a>
            </div>
          ...
          <s:if test="commentsWithStatusPage.pageCnt>1">
					<div class="W_pages_minibtn">
						<s:if test="commentsWithStatusPage.befPageNum>0 && commentsWithStatusPage.curPageNum!=1">      	
						   <a href="/commentIn.jspa?page=<s:property value="commentsWithStatusPage.befPageNum" />" class="btn_page">上一页</a>
						</s:if>
						<s:iterator value="commentsWithStatusPage.pageList" id="curPage">
							...
							<s:else>
								<a href="/commentIn.jspa?page=<s:property value="#curPage" />"><s:property value="#curPage" /></a>
							</s:else>
						</s:iterator>
						   <s:if test="commentsWithStatusPage.curPageNum!=commentsWithStatusPage.pageCnt">
						   <a href="/commentIn.jspa?page=<s:property value="commentsWithStatusPage.aftPageNum" />" class="btn_page">下一页</a>
						   </s:if>
					</div>
			</s:if>
        </div>
